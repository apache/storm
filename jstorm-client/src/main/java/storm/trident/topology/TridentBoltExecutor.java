...
public class TridentBoltExecutor implements IRichBolt {
	...
	public TridentBoltExecutor(ITridentBatchBolt bolt,
	...
	public void prepare(Map conf, TopologyContext context,
		...
		_coordOutputCollector = new BatchOutputCollectorImpl(
				new OutputCollector(_coordCollector));
			...
			for (String batchGroup : _coordSpecs.keySet()) {
				...
				for (String comp : spec.coords.keySet()) {
					...
					if (ct.equals(CoordType.single())) {
						...
						cond.expectedTaskReports += context.getComponentTasks(
								comp).size();
					}
				}
				...
				for (String component : Utils.get(context.getThisTargets(),
					...
					cond.targetTasks.addAll(context
		...
		try {
			...
			for (Integer task : tracked.condition.targetTasks) {
				_collector.emitDirect(
						...
						stream,
						finishTuple,
						new Values(tracked.info.batchId, Utils.get(
								tracked.taskEmittedTuples, task, 0)));
			}
			if (tracked.delayedAck != null) {
	...
	private void checkFinish(TrackedBatch tracked, Tuple tuple, TupleType type) {
		...
		boolean delayed = tracked.delayedAck == null
				&& (cond.commitStream != null && type == TupleType.COMMIT || cond.commitStream == null);
		if (delayed) {
			tracked.delayedAck = tuple;
		}
		boolean failed = false;
		if (tracked.receivedCommit
				&& tracked.reportedTasks == cond.expectedTaskReports) {
	...
	public void execute(Tuple tuple) {
		...
		// get transaction id
		// if it already exissts and attempt id is greater than the attempt
		// there

		TrackedBatch tracked = (TrackedBatch) _batches.get(id.getId());
		// if(_batches.size() > 10 && _context.getThisTaskIndex() == 0) {
		// System.out.println("Received in " + _context.getThisComponentId() +
		// " " + _context.getThisTaskIndex()
		// + " (" + _batches.size() + ")" +
		// "\ntuple: " + tuple +
		...
		//
		// }
		// System.out.println("Num tracked: " + _batches.size() + " " +
		// _context.getThisComponentId() + " " + _context.getThisTaskIndex());

		// this code here ensures that only one attempt is ever tracked for a
		// batch, so when
		// failures happen you don't get an explosion in memory usage in the
		// tasks
		if (tracked != null) {
		...
		if (tracked == null) {
			tracked = new TrackedBatch(new BatchInfo(batchGroup, id,
					_bolt.initBatchState(batchGroup, id)),
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		...
		for (String batchGroup : _coordSpecs.keySet()) {
			declarer.declareStream(COORD_STREAM(batchGroup), true, new Fields(
					"id", "count"));
		}
	}
	...
	public Map<String, Object> getComponentConfiguration() {
		...
		// TODO: Need to be able to set the tick tuple time to the message
		// timeout, ideally without parameterization
		return ret;
	}
