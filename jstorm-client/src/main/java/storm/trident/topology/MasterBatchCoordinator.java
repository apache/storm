...
public class MasterBatchCoordinator extends BaseRichSpout {
	public static final Logger LOG = LoggerFactory
			.getLogger(MasterBatchCoordinator.class);
	...
	public MasterBatchCoordinator(List<String> spoutIds,
	...
	public void open(Map conf, TopologyContext context,
		...
		_throttler = new WindowedTimeThrottler(
				(Number) conf
						.get(Config.TOPOLOGY_TRIDENT_BATCH_EMIT_INTERVAL_MILLIS),
				1);
		...
		_attemptIds = getStoredCurrAttempts(_currTransaction,
			...
			if (_activeTx.size() < _maxTransactionActive) {
				...
				for (int i = 0; i < _maxTransactionActive; i++) {
					if (!_activeTx.containsKey(curr) && isReady(curr)) {
						// by using a monotonically increasing attempt id,
						// downstream tasks
						// can be memory efficient by clearing out state for old
						// attempts
						// as soon as they see a higher attempt id for a
						// transaction
	...
	private boolean isReady(long txid) {
		if (_throttler.isThrottled())
			return false;
		// TODO: make this strategy configurable?... right now it goes if anyone
		// is ready
	...
	private TreeMap<Long, Integer> getStoredCurrAttempts(long currTransaction,
		...
		for (TransactionalState state : _states) {
			Map<Object, Number> attempts = (Map) state
					.getData(CURRENT_ATTEMPTS);
			if (attempts == null)
