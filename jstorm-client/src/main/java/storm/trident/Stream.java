...
public class Stream implements IAggregatableStream {
	...
	public Stream project(Fields keepFields) {
		...
		return _topology.addSourcedNode(this,
				new ProcessorNode(_topology.getUniqueStreamId(), _name,
						keepFields, new Fields(), new ProjectedProcessor(
								keepFields)));
	}
	...
	public Stream partition(CustomStreamGrouping partitioner) {
		return partition(Grouping.custom_serialized(Utils
	...
	public Stream partition(Grouping grouping) {
		if (_node instanceof PartitionNode) {
			...
			return _topology.addSourcedNode(this, new PartitionNode(
	...
	@Override
	public Stream each(Fields inputFields, Function function,
	...
	@Override
	public Stream partitionAggregate(Fields inputFields, Aggregator agg,
			Fields functionFields) {
		projectionValidation(inputFields);
		return _topology.addSourcedNode(this,
				new ProcessorNode(_topology.getUniqueStreamId(), _name,
						functionFields, functionFields, new AggregateProcessor(
								inputFields, agg)));
	}
	...
	public Stream stateQuery(TridentState state, Fields inputFields,
		...
		Node n = new ProcessorNode(_topology.getUniqueStreamId(), _name,
				TridentUtils.fieldsConcat(getOutputFields(), functionFields),
				functionFields, new StateQueryProcessor(stateId, inputFields,
						function));
	...
	public TridentState partitionPersist(StateFactory stateFactory,
		...
		return partitionPersist(new StateSpec(stateFactory), inputFields,
				updater, functionFields);
	}
	...
	public TridentState partitionPersist(StateSpec stateSpec,
		...
		ProcessorNode n = new ProcessorNode(_topology.getUniqueStreamId(),
				_name, functionFields, functionFields,
	...
	public TridentState partitionPersist(StateFactory stateFactory,
		...
		return partitionPersist(stateFactory, inputFields, updater,
				new Fields());
	}
	...
	public Stream partitionAggregate(Fields inputFields, ReducerAggregator agg,
			Fields functionFields) {
		projectionValidation(inputFields);
		return chainedAgg()
				.partitionAggregate(inputFields, agg, functionFields)
	...
	public Stream aggregate(Fields inputFields, Aggregator agg,
			Fields functionFields) {
		projectionValidation(inputFields);
		return chainedAgg().aggregate(inputFields, agg, functionFields)
				.chainEnd();
	}
	...
	public Stream aggregate(Fields inputFields, CombinerAggregator agg,
			Fields functionFields) {
		projectionValidation(inputFields);
		return chainedAgg().aggregate(inputFields, agg, functionFields)
				.chainEnd();
	...
	public Stream aggregate(Fields inputFields, ReducerAggregator agg,
			Fields functionFields) {
		projectionValidation(inputFields);
		return chainedAgg().aggregate(inputFields, agg, functionFields)
				.chainEnd();
	...
	public TridentState partitionPersist(StateFactory stateFactory,
		...
		return partitionPersist(new StateSpec(stateFactory), updater,
				functionFields);
	}
	...
	public TridentState persistentAggregate(StateFactory stateFactory,
			CombinerAggregator agg, Fields functionFields) {
		return persistentAggregate(new StateSpec(stateFactory), agg,
				functionFields);
	}
	...
	public TridentState persistentAggregate(StateFactory stateFactory,
		...
		return persistentAggregate(new StateSpec(stateFactory), inputFields,
				agg, functionFields);
	}
	...
	public TridentState persistentAggregate(StateSpec spec, Fields inputFields,
		...
		return new ChainedAggregatorDeclarer(this, new GlobalAggScheme())
				...
				.chainEnd()
				.partitionPersist(spec, functionFields,
						new CombinerAggStateUpdater(agg), functionFields);
	}
	...
	public TridentState persistentAggregate(StateSpec spec, Fields inputFields,
			ReducerAggregator agg, Fields functionFields) {
		projectionValidation(inputFields);
		return global().partitionPersist(spec, inputFields,
				new ReducerAggStateUpdater(agg), functionFields);
	}

	public Stream stateQuery(TridentState state, QueryFunction function,
			Fields functionFields) {
	...
	static class BatchGlobalAggScheme implements
			GlobalAggregationScheme<Stream> {
		...
		for (String field : projFields) {
			if (!allFields.contains(field)) {
				throw new IllegalArgumentException(
						"Trying to select non-existent field: '" + field
