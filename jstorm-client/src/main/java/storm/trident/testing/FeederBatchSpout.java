...
public class FeederBatchSpout implements ITridentSpout, IFeeder {
	...
	public FeederBatchSpout(List<String> fields) {
		...
		_semaphoreId = RegisteredGlobalState
				.registerState(new CopyOnWriteArrayList());
	}
	...
	public class FeederCoordinator implements
		...
		@Override
		public Map<Integer, List<List<Object>>> initializeTransaction(
				long txid, Map<Integer, List<List<Object>>> prevMetadata,
		...
		public void success(long txid) {
			...
			if (index != null) {
				Semaphore sem = (Semaphore) ((List) RegisteredGlobalState
						.getState(_semaphoreId)).get(index);
				sem.release();
			}
		}
	...
	public class FeederEmitter implements
	...
	public BatchCoordinator getCoordinator(String txStateId, Map conf,
		...
		int numTasks = context.getComponentTasks(
				TridentTopologyBuilder.spoutIdFromCoordinatorId(context
						.getThisComponentId())).size();
