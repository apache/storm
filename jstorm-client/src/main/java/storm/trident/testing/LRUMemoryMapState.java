...
public class LRUMemoryMapState<T> implements Snapshottable<T>,
		ITupleCollection, MapState<T> {
	...
	public LRUMemoryMapState(int cacheSize, String id) {
		...
		_delegate = new SnapshottableMap(OpaqueMap.build(_backing), new Values(
				"$MEMORY-MAP-STATE-GLOBAL$"));
	}
	...
	static class LRUMemoryMapStateBacking<T> implements IBackingMap<T>,
			ITupleCollection {
		...
		public Iterator<List<Object>> getTuples() {
			return new Iterator<List<Object>>() {
				...
				private Iterator<Map.Entry<List<Object>, T>> it = db.entrySet()
						.iterator();
