...
public class PartitionPersistProcessor implements TridentProcessor {
	...
	public PartitionPersistProcessor(String stateId, Fields inputFields,
	...
	public void prepare(Map conf, TopologyContext context,
		...
		_updater.prepare(conf,
				new TridentOperationContext(context, _projection));
	}
	...
	public void execute(ProcessorContext processorContext, String streamId,
			TridentTuple tuple) {
		((List) processorContext.state[_context.getStateIndex()])
				.add(_projection.create(tuple));
	}
	...
	public void finishBatch(ProcessorContext processorContext) {
		...
		// since this processor type is a committer, this occurs in the commit
		// phase
		List<TridentTuple> buffer = (List) processorContext.state[_context
				.getStateIndex()];

		// don't update unless there are tuples
		// this helps out with things like global partition persist, where
		// multiple tasks may still
		// exist for this processor. Only want the global one to do anything
		// this is also a helpful optimization that state implementations don't
		// need to manually do
		if (buffer.size() > 0) {
			Long txid = null;
			// this is to support things like persisting off of drpc stream,
