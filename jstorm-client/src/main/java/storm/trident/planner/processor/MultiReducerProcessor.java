...
public class MultiReducerProcessor implements TridentProcessor {
	...
	public void prepare(Map conf, TopologyContext context,
		...
		for (int i = 0; i < _projectFields.size(); i++) {
			_projectionFactories[i] = new ProjectionFactory(parents.get(i),
					_projectFields.get(i));
		}
		_collector = new FreshCollector(tridentContext);
		_reducer.prepare(
				conf,
				new TridentMultiReducerContext((List) Arrays
						.asList(_projectionFactories)));
	}
	...
	public void startBatch(ProcessorContext processorContext) {
		...
		processorContext.state[_context.getStateIndex()] = _reducer
				.init(_collector);
	}
	...
	public void finishBatch(ProcessorContext processorContext) {
		_collector.setContext(processorContext);
		_reducer.complete(processorContext.state[_context.getStateIndex()],
				_collector);
	}
