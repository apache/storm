...
public class StateQueryProcessor implements TridentProcessor {
	...
	public StateQueryProcessor(String stateId, Fields inputFields,
	...
	public void prepare(Map conf, TopologyContext context,
		...
		_function.prepare(conf, new TridentOperationContext(context,
				_projection));
	}
	...
	public void execute(ProcessorContext processorContext, String streamId,
			TridentTuple tuple) {
		BatchState state = (BatchState) processorContext.state[_context
				.getStateIndex()];
	...
	public void finishBatch(ProcessorContext processorContext) {
		...
		if (!state.tuples.isEmpty()) {
			...
			if (results.size() != state.tuples.size()) {
				throw new RuntimeException(
						"Results size is different than argument size: "
								+ results.size() + " vs " + state.tuples.size());
			}
			for (int i = 0; i < state.tuples.size(); i++) {
				...
				_function
						.execute(_projection.create(tuple), result, _collector);
			}
		}
	}
