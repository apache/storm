...
public interface TridentProcessor extends Serializable, TupleReceiver {
	...
	// imperative that don't emit any tuples from here, since output factory
	// cannot be gotten until
	// preparation is done, therefore, receivers won't be ready to receive
	// tuples yet
	// can't emit tuples from here anyway, since it's not within a batch context
	// (which is only
