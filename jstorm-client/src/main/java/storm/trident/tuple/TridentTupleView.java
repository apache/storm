...
public class TridentTupleView extends AbstractList<Object> implements
		TridentTuple {
	ValuePointer[] _index;
		...
		public TridentTuple create(TridentTuple parent) {
			if (_index.length == 0)
				...
				return new TridentTupleView(
						((TridentTupleView) parent)._delegates, _index,
						_fieldIndex);
		}
	...
	public static class FreshOutputFactory implements Factory {
		...
		public TridentTuple create(List<Object> selfVals) {
			return new TridentTupleView(PersistentVector.EMPTY.cons(selfVals),
					_index, _fieldIndex);
		}
	...
	public static class RootFactory implements Factory {
		...
		public TridentTuple create(Tuple parent) {
			return new TridentTupleView(PersistentVector.EMPTY.cons(parent
					.getValues()), index, fieldIndex);
		}
	...
	public static TridentTupleView EMPTY_TUPLE = new TridentTupleView(null,
			new ValuePointer[0], new HashMap());

	// index and fieldIndex are precomputed, delegates built up over many
	// operations using persistent data structures
	public TridentTupleView(IPersistentVector delegates, ValuePointer[] index,
			Map<String, ValuePointer> fieldIndex) {
	...
	private Object getValueByPointer(ValuePointer ptr) {
		return ((List<Object>) _delegates.nth(ptr.delegateIndex))
				.get(ptr.index);
	}
}
