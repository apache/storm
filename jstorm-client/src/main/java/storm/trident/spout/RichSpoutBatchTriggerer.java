...
public class RichSpoutBatchTriggerer implements IRichSpout {
	...
	public RichSpoutBatchTriggerer(IRichSpout delegate, String streamName,
			String batchGroup) {
		_delegate = delegate;
		_stream = streamName;
	...
	public void open(Map conf, TopologyContext context,
		...
		_delegate.open(conf, context, new SpoutOutputCollector(
				new StreamOverrideCollector(collector)));
		_outputTasks = new ArrayList<Integer>();
		for (String component : Utils.get(context.getThisTargets(),
				_coordStream, new HashMap<String, Grouping>()).keySet()) {
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		...
		// try to find a way to merge this code with what's already done in
		// TridentBoltExecutor
	...
	public Map<String, Object> getComponentConfiguration() {
		...
		Config.registerSerialization(conf, RichSpoutBatchId.class,
				RichSpoutBatchIdSerializer.class);
		return conf;
	}
	...
	class StreamOverrideCollector implements ISpoutOutputCollector {
		...
		@Override
		public void emitDirect(int task, String ignore, List<Object> values,
				Object msgId) {
