...
public interface IPartitionedTridentSpout<Partitions, Partition extends ISpoutPartition, T>
	...
	public interface Coordinator<Partitions> {
		/**
		 * Return the partitions currently in the source of data. The idea is is
		 * that if a new partition is added and a prior transaction is replayed,
		 * it doesn't emit tuples for the new partition because it knows what
		 * partitions were in that transaction.
		 */
	...
	public interface Emitter<Partitions, Partition extends ISpoutPartition, X> {
		 ...
		 * partitions. Should be used to manage things like connections to
		 * brokers.
		 */
		...
		void emitPartitionBatch(TransactionAttempt tx,
				TridentCollector collector, Partition partition, X partitionMeta);
	...
	Emitter<Partitions, Partition, T> getEmitter(Map conf,
