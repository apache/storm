...
public class OpaquePartitionedTridentSpoutExecutor implements
	...
	static class EmitterPartitionState {
		...
		public EmitterPartitionState(RotatingTransactionalState s,
				ISpoutPartition p) {
			rotatingState = s;
			partition = p;
		}
	...
	public class Emitter implements ICommitterTridentSpout.Emitter {
		...
		public void emitBatch(TransactionAttempt tx, Object coordinatorMeta,
			...
			if (_savedCoordinatorMeta == null
				...
				List<ISpoutPartition> partitions = _emitter
						.getOrderedPartitions(coordinatorMeta);
				...
				for (int i = _index; i < partitions.size(); i += _numTasks) {
					...
					String id = p.getId();
					myPartitions.add(p);
					_partitionStates.put(id, new EmitterPartitionState(
							new RotatingTransactionalState(_state, id), p));
				}
			...
			Entry<Long, Map<String, Object>> entry = _cachedMetas.lowerEntry(tx
					.getTransactionId());
		...
		public void commit(TransactionAttempt attempt) {
			// this code here handles a case where a previous commit failed, and
			// the partitions
			// changed since the last commit. This clears out any state for the
			// removed partitions
			// for this txid.
			// we make sure only a single task ever does this. we're also
			// guaranteed that
			// it's impossible for there to be another writer to the directory
			// for that partition
			// because only a single commit can be happening at once. this is
			// because in order for
			// another attempt of the batch to commit, the batch phase must have
			// succeeded in between.
			// hence, all tasks for the prior commit must have finished
			// committing (whether successfully or not)
			if (_changedMeta && _index == 0) {
				Set<String> validIds = new HashSet<String>();
				for (ISpoutPartition p : (List<ISpoutPartition>) _emitter
						.getOrderedPartitions(_savedCoordinatorMeta)) {
					validIds.add(p.getId());
				}
				for (String existingPartition : _state.list("")) {
					if (!validIds.contains(existingPartition)) {
						RotatingTransactionalState s = new RotatingTransactionalState(
								_state, existingPartition);
			...
			for (String partitionId : metas.keySet()) {
				...
				_partitionStates.get(partitionId).rotatingState.overrideState(
						txid, meta);
			}
		}
	...
	public OpaquePartitionedTridentSpoutExecutor(
			IOpaquePartitionedTridentSpout spout) {
		_spout = spout;
	}

	@Override
	public ITridentSpout.BatchCoordinator<Object> getCoordinator(
			String txStateId, Map conf, TopologyContext context) {
		return new Coordinator(conf, context);
	}

	@Override
	public ICommitterTridentSpout.Emitter getEmitter(String txStateId,
