...
public class IdentityGrouping implements CustomStreamGrouping {
	...
	public void prepare(WorkerTopologyContext context, GlobalStreamId stream,
			List<Integer> tasks) {
		List<Integer> sourceTasks = new ArrayList<Integer>(
				context.getComponentTasks(stream.get_componentId()));
	...
	public List<Integer> chooseTasks(int task, List<Object> values) {
		...
		if (ret == null) {
			throw new RuntimeException(
					"Tuple emitted by task that's not part of this component. Should be impossible");
		}
		return ret;
	}
