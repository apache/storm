...
public class MicroBatchIBackingMap<T> implements IBackingMap<T> {
	...
	public static class Options implements Serializable {
		public int maxMultiGetBatchSize = 0; // 0 means delegate batch size =
												// trident batch size.
		public int maxMultiPutBatchSize = 0;
	}

	public MicroBatchIBackingMap(final Options options,
			final IBackingMap<T> delegate) {
		_options = options;
		_delegate = delegate;
	...
	public void multiPut(final List<List<Object>> keys, final List<T> values) {
		...
		while (!keysTodo.isEmpty()) {
			List<List<Object>> keysBatch = new ArrayList<List<Object>>(
					thisBatchSize);
