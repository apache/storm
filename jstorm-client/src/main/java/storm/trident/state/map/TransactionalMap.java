...
public class TransactionalMap<T> implements MapState<T> {
	...
	public List<T> multiGet(List<List<Object>> keys) {
		List<CachedBatchReadsMap.RetVal<TransactionalValue>> vals = _backing
				.multiGet(keys);
	...
	public List<T> multiUpdate(List<List<Object>> keys,
		...
		List<CachedBatchReadsMap.RetVal<TransactionalValue>> curr = _backing
				.multiGet(keys);
		List<TransactionalValue> newVals = new ArrayList<TransactionalValue>(
				curr.size());
		...
		for (int i = 0; i < curr.size(); i++) {
			...
			if (val == null) {
				newVal = new TransactionalValue<T>(_currTx,
						updater.update(null));
				changed = true;
			} else {
