...
public class MapCombinerAggStateUpdater implements StateUpdater<MapState> {
	...
	public MapCombinerAggStateUpdater(CombinerAggregator agg,
			Fields groupFields, Fields inputFields) {
		_agg = agg;
	...
	@Override
	public void updateState(MapState map, List<TridentTuple> tuples,
		...
		for (TridentTuple t : tuples) {
			...
			updaters.add(new CombinerValueUpdater(_agg, _inputFactory.create(t)
					.getValue(0)));
		}
		...
		for (int i = 0; i < tuples.size(); i++) {
			...
			collector.emit(_factory
					.create(new List[] { key, new Values(result) }));
		}
	}
