...
public class ChainedAggregatorImpl implements Aggregator<ChainedResult> {
	...
	public ChainedAggregatorImpl(Aggregator[] aggs, Fields[] inputFields,
	...
	public void prepare(Map conf, TridentOperationContext context) {
		...
		for (int i = 0; i < _inputFields.length; i++) {
			...
			_aggs[i].prepare(conf, new TridentOperationContext(context,
					_inputFactories[i]));
		}
	}
	...
	public void aggregate(ChainedResult val, TridentTuple tuple,
			TridentCollector collector) {
		val.setFollowThroughCollector(collector);
		for (int i = 0; i < _aggs.length; i++) {
			TridentTuple projected = _inputFactories[i]
					.create((TridentTupleView) tuple);
	...
	public void complete(ChainedResult val, TridentCollector collector) {
		...
		if (_aggs.length > 1) { // otherwise, tuples were emitted directly
			...
			while (keepGoing) {
				...
				keepGoing = increment(val.collectors, indices,
						indices.length - 1);
			}
		}
	}
