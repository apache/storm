...
public class JoinerMultiReducer implements GroupedMultiReducer<JoinState> {
	...
	public JoinerMultiReducer(List<JoinType> types, int numGroupFields,
			List<Fields> sides) {
		_types = types;
		_sideFields = sides;
	...
	@Override
	public void execute(JoinState state, int streamIndex, TridentTuple group,
			TridentTuple input, TridentCollector collector) {
		// TODO: do the inner join incrementally, emitting the cross join with
		// this tuple, against all other sides
	...
	@Override
	public void complete(JoinState state, TridentTuple group,
	...
	private void emitCrossJoin(JoinState state, TridentCollector collector,
			int overrideIndex, TridentTuple overrideTuple) {
		List<List>[] sides = state.sides;
		...
		while (keepGoing) {
			...
			keepGoing = increment(sides, indices, indices.length - 1,
					overrideIndex);
		}
	}
	...
	private boolean increment(List[] lengths, int[] indices, int j,
			int overrideIndex) {
		if (j == -1)
			return false;
		if (j == overrideIndex) {
