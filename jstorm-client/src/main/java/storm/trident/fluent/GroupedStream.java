...
public class GroupedStream implements IAggregatableStream,
	...
	public Stream aggregate(Fields inputFields, Aggregator agg,
		...
		return new ChainedAggregatorDeclarer(this, this).aggregate(inputFields,
				agg, functionFields).chainEnd();
	}
	...
	public TridentState persistentAggregate(StateSpec spec, Fields inputFields,
			CombinerAggregator agg, Fields functionFields) {
		return aggregate(inputFields, agg, functionFields).partitionPersist(
				spec,
				TridentUtils.fieldsUnion(_groupFields, functionFields),
				new MapCombinerAggStateUpdater(agg, _groupFields,
						functionFields),
	...
	public TridentState persistentAggregate(StateSpec spec, Fields inputFields,
		...
		return _stream.partitionBy(_groupFields).partitionPersist(spec,
	...
	public Stream stateQuery(TridentState state, Fields inputFields,
		...
		return _stream.partitionBy(_groupFields).stateQuery(state, inputFields,
	...
	@Override
	public IAggregatableStream each(Fields inputFields, Function function,
			Fields functionFields) {
	...
	public IAggregatableStream partitionAggregate(Fields inputFields,
		...
		Aggregator groupedAgg = new GroupedAggregator(agg, _groupFields,
				inputFields, functionFields.size());
		Fields allInFields = TridentUtils
				.fieldsUnion(_groupFields, inputFields);
		Fields allOutFields = TridentUtils.fieldsConcat(_groupFields,
				functionFields);
		Stream s = _stream.partitionAggregate(allInFields, groupedAgg,
				allOutFields);
		return new GroupedStream(s, _groupFields);
	}
	...
	public IAggregatableStream aggPartition(GroupedStream s) {
		return new GroupedStream(s._stream.partitionBy(_groupFields),
				_groupFields);
	}
