...
This tutorial uses examples from the [storm-starter](https://github.com/apache/storm/blob/master/examples/storm-starter) project. It's recommended that you clone the project and follow along with the examples. Read [Setting up a development environment](/documentation/Setting-up-development-environment.html) and [Creating a new Storm project](/documentation/Creating-a-new-Storm-project.html) to get your machine set up.
...
![Storm cluster](/documentation/images/storm-cluster.png)
...
![A Storm topology](/documentation/images/topology.png)
...
The object containing the processing logic implements the [IRichSpout](/javadoc/apidocs/org/apache/storm/topology/IRichSpout.html) interface for spouts and the [IRichBolt](/javadoc/apidocs/org/apache/storm/topology/IRichBolt.html) interface for bolts.
...
The `cleanup` method is called when a Bolt is being shutdown and should cleanup any resources that were opened. There's no guarantee that this method will be called on the cluster: for example, if the machine the task is running on blows up, there's no way to invoke the method. The `cleanup` method is intended for when you run topologies in [local mode](/documentation/Local-mode.html) (where a Storm cluster is simulated in process), and you want to be able to run and kill many topologies without suffering any resource leaks.

The `declareOutputFields` method declares that the `ExclamationBolt` emits 1-tuples with one field called "word".

The `getComponentConfiguration` method allows you to configure various aspects of how this component runs. This is a more advanced topic that is explained further on [Configuration](/documentation/Configuration.html).
...
There's many other configurations you can set for the topology. The various configurations are detailed on [the Javadoc for Config](/javadoc/apidocs/org/apache/storm/Config.html).

To learn about how to set up your development environment so that you can run topologies in local mode (such as in Eclipse), see [Creating a new Storm project](/documentation/Creating-a-new-Storm-project.html).
...
![Tasks in a topology](/documentation/images/topology-tasks.png)
...
There's a few other kinds of stream groupings. You can read more about them on [Concepts](/documentation/Concepts.html). 
