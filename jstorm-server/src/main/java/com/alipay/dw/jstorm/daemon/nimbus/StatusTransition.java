...
import com.alipay.dw.jstorm.callback.Callback;
import com.alipay.dw.jstorm.callback.impl.ActiveTransitionCallback;
import com.alipay.dw.jstorm.callback.impl.DoRebalanceTransitionCallback;
import com.alipay.dw.jstorm.callback.impl.InactiveTransitionCallback;
import com.alipay.dw.jstorm.callback.impl.KillTransitionCallback;
import com.alipay.dw.jstorm.callback.impl.ReassignTransitionCallback;
import com.alipay.dw.jstorm.callback.impl.RebalanceTransitionCallback;
import com.alipay.dw.jstorm.callback.impl.RemoveTransitionCallback;
...
public class StatusTransition {
     ...
     * 
     */
    public <T> void transitionLock(String topologyid, boolean errorOnNoTransition,
            StatusType changeStatus, T... args) throws Exception {
        ...
        if (changingCallbacks == null
            ...
            String msg = "No transition for event: changing status"
                    ...
                    + currentStatus.getStatusType() + " topology-id: " + topologyid;
            LOG.info(msg);
    ...
    private Map<StatusType, Map<StatusType, Callback>> stateTransitions(
         ...
         * killed/inactive/active/rebalancing
         * 2. action:
         * 
         * monitor -- every Config.NIMBUS_MONITOR_FREQ_SECS seconds will trigger
         * this
         * only valid when current status is active
         * inactivate -- client will trigger this action,
         * only valid when current status is active
         * activate -- client will trigger this action
         * only valid when current status is inactive
         * startup -- when nimbus startup, it will trigger this action
         * only valid when current status is killed/rebalancing
         * kill -- client kill topology will trigger this action,
         * only valid when current status is active/inactive/killed
         * remove -- 30 seconds after client submit kill command,
         * it will do this action, only valid when current status is killed
         * rebalance -- client submit rebalance command,
         * only valid when current status is active/deactive
         * do_rebalance -- 30 seconds after client submit rebalance command,
         * it will do this action,
         * only valid when current status is rebalance
         */
        ...
        inactiveMap.put(StatusType.monitor, new ReassignTransitionCallback(data,
                topologyid, new StormStatus(StatusType.inactive)));
