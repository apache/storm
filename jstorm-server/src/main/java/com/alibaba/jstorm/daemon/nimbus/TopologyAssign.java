...
public class TopologyAssign implements Runnable {
    ...
    public void setTopologyStatus(TopologyAssignEvent event) throws Exception {
        ...
        if (stormBase == null) {
            stormBase = new StormBase(topologyName, TimeUtils.current_time_secs(), status);
    ...
    protected TopologyAssignContext prepareTopologyAssign(TopologyAssignEvent event)
        ...
        StormTopology rawTopology = StormConfig.read_nimbus_topology_code(conf, topologyId);
        ret.setRawTopology(rawTopology);

        Map stormConf = new HashMap();
        stormConf.putAll(conf);
        stormConf.putAll(StormConfig.read_nimbus_topology_conf(conf, topologyId));
        ...
        if (supInfos.size() == 0) {
            throw new FailedAssignTopologyException("Failed to make assignment " + topologyId
                                                    + ", due to no alive supervisor");
        }

        Map<Integer, String> taskToComponent = Cluster.topology_task_info(stormClusterState,
            topologyId);
    ...
    public Assignment mkAssignment(TopologyAssignEvent event) throws Exception {
        ...
        Map<String, String> nodeHost = getTopologyNodeHost(context.getCluster(),
            context.getOldAssignment(), taskAssignments);

        Map<Integer, Integer> startTimes = getTaskStartTimes(context, nimbusData, topologyId,
            context.getOldAssignment(), taskAssignments);
     ...
     * @throws Exception 
     */
    public static Map<Integer, Integer> getTaskStartTimes(TopologyAssignContext context,
    ...
    public static Map<String, String> getTopologyNodeHost(Map<String, SupervisorInfo> supervisorMap,
                                                          Assignment existingAssignment,
                                                          Map<Integer, ResourceAssignment> taskWorkerSlot) {

        // the following is that remove unused node from allNodeHost
        Set<String> usedNodes = new HashSet<String>();
        for (Entry<Integer, ResourceAssignment> entry : taskWorkerSlot.entrySet()) {
     ...
     */
    public static Set<Integer> getChangeTaskIds(Map<Integer, ResourceAssignment> oldTaskToWorkerSlot,
     ...
     * 
     * @@@ Question
     * Do we need to do backup operation every time?
     * 
     * @param assignment
     * @param event
     */
