...
//        LOG.debug("{} send/recv time (ms): {}", recvCounter.incrementAndGet(),
...
//            trybatchCounter.incrementAndGet(), trymsgCounter.addAndGet(requests.size()));
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, ExceptionEvent event) {
        ...
        if (being_closed.get() == false) {
            if (!(cause instanceof ConnectException)) {
                LOG.info("Connection failed:" + client.getTarget_Server(), cause);
            }
