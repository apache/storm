...
public class AssignTaskUtils {
    private static final Logger LOG                                     = Logger
            .getLogger(AssignTaskUtils.class);
    
    public static boolean checkResourceValid(ResourceAssignment taskAlloc, Integer task,
        ...
        // because dead task's old assignment can't pass isSupervisorAvailable check
        //        Set<Integer> deadTasks = defaultContext.getDeadTaskIds();
        //        if (deadTasks.contains(task)) {
        //            LOG.info(componentName + " task " + task + " is dead, won't use old assignment");
        //            return false;
        //        }
        ...
        if (supervisorInfo == null) {
            LOG.info(componentName + " task " + task + " supervisorId is invalid "
                     + allocSupervisorId);
            return false;
        }
        ...
        if (isIsolate(task, allocSupervisorId, componentName, componentMap, defaultContext,
            alreadyAssign) == false) {
            StringBuilder sb = new StringBuilder();
            sb.append("Supervisor " + allocSupervisorId);
    ...
    public static boolean isSupervisorAvailable(DefaultTopologyAssignContext defaultContext,
                                          SupervisorInfo supervisorInfo,
                                          ResourceAssignment taskAlloc) {
        boolean isCpuAvailable = supervisorInfo.getCpuPool().isAvailable(taskAlloc.getCpuSlotNum(),
            defaultContext);
        if (isCpuAvailable == false) {
            return false;
        }
        boolean isMemAvailable = supervisorInfo.getMemPool().isAvailable(taskAlloc.getMemSlotNum(),
            defaultContext);
        if (isMemAvailable == false) {
            return false;
        }
        boolean isDiskAvailable = supervisorInfo.getDiskPool().isAvailable(taskAlloc.getDiskSlot(),
            defaultContext);
        if (isDiskAvailable == false) {
            return false;
        }
        boolean isPortAvailable = supervisorInfo.getNetPool().isAvailable(taskAlloc.getPort(),
            defaultContext);
    ...
    public static void allocResource(DefaultTopologyAssignContext defaultContext,
                               ResourceAssignment taskAlloc) {
        Map<String, SupervisorInfo> cluster = defaultContext.getCluster();
    ...
    public static ResourceAssignment allocResource(DefaultTopologyAssignContext defaultContext,
    ...
    /**
     * if the task has been set TASK_RUN_ON_DIFFERENT_NODE
     * the component's task should be run on different node
     * 
     * @param task
     ...
     * @return
     */
    public static boolean isIsolate(Integer task, String assignSupervisorId, String componentName,
                              Map componentMap, DefaultTopologyAssignContext defaultContext,
                              Map<Integer, ResourceAssignment> alreadyAssign) {
    ...
    public static ResourceAssignment normalizeResourceAssignment(DefaultTopologyAssignContext defaultContext,
                                                           ResourceAssignment taskAlloc,
    ...
    public static  ResourceAssignment getUserDefineAlloc(Integer task,
        ...
        ResourceAssignment taskAlloc = normalizeResourceAssignment(defaultContext,
            userDefineAssign, checkHostname);
        if (taskAlloc == null) {
            LOG.info(componentName + " index task " + index + " ResourceAssignment is invalid "
    ...
    public static TaskAllocResource getTaskAlloc(Integer task,
        ...
        ResourceAssignment useDefineAlloc = getUserDefineAlloc(task, defaultContext, componentMap,
            componentName, false);
