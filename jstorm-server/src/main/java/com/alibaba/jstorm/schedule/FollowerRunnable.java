...
import com.alibaba.jstorm.zk.WatcherCallBack;
...
public class FollowerRunnable implements Runnable{
	...
	public FollowerRunnable(NimbusData data, LeaderSelector leaderSelector, int sleepTime) {
		this.data = data;
	...
	private void downloadCodeFromMaster(Assignment assignment, String topologyId) throws IOException, TException {
		try {
			...
			JStormServerUtils.downloadCodeFromMaster(data.getConf(), localRoot, masterCodeDir);
		} catch (TException e) {
			// TODO Auto-generated catch block
			LOG.error(e + " downloadStormCode failed " + "stormId:"
                    + topologyId + "masterCodeDir:" + assignment.getMasterCodeDir());
            throw e;
		}
