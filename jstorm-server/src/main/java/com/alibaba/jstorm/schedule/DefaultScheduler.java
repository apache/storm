...
public class DefaultScheduler implements IScheduler {
	...
	private Set<WorkerSlot> slotsCanReassign(Cluster cluster,
		...
		for (WorkerSlot slot : slots) {
			if (!cluster.isBlackListed(slot.getNodeId())) {
				if (cluster.getSupervisorById(slot.getNodeId()).getAllPorts()
						.contains(slot.getPort()))
					result.add(slot);
			}
		}
		return result;
	...
	private Set<WorkerSlot> badSlots(
			Map<WorkerSlot, List<ExecutorDetails>> existingSlots,
			int numExecutors, int numWorkers) {
		if (numWorkers != 0) {
			Map<Integer, Integer> distribution = Utils.integerDivided(
					numExecutors, numWorkers);
			...
			for (Entry<WorkerSlot, List<ExecutorDetails>> entry : existingSlots
					.entrySet()) {
				Integer executorCount = distribution.get(entry.getValue()
						.size());
	...
	public void schedule(Topologies topologies, Cluster cluster) {
		...
		for (TopologyDetails details : needsSchedulingTopologies) {
			...
			Set<ExecutorDetails> allExecutors = (Set<ExecutorDetails>) details
					.getExecutors();
			Map<WorkerSlot, List<ExecutorDetails>> aliveAssigned = EvenScheduler
					.getAliveAssignedWorkerSlotExecutors(cluster,
							details.getId());
			...
			int totalSlotsToUse = Math.min(details.getNumWorkers(),
			...
			EvenScheduler.scheduleTopologiesEvenly(new Topologies(
					topologiesCache), cluster);
		}
	}
