...
public class UseOldPreAssign implements IPreassignTask {
	private static final Logger LOG = Logger.getLogger(UseOldPreAssign.class);
	...
	private ResourceAssignment getOldAlloc(Integer task,
		...
		ResourceAssignment oldAssignment = oldAssign.getTaskToResource().get(
				task);
		...
		if (taskAlloc.allocEqual(oldAssignment) == false) {
			LOG.info(componentName + " of " + task
					+ " allocation has changed, old assignment can't be used ");
	...
	public ResourceAssignment preAssign(Integer task,
		...
		ResourceAssignment oldAssignment = getOldAlloc(task, defaultContext,
				componentMap, componentName);
		if (oldAssignment == null) {
			return null;
		}

		boolean resourceValid = AssignTaskUtils.checkResourceValid(
				oldAssignment, task, defaultContext, componentMap,
				componentName, canUsedSupervisorIds, alreadyAssign);
		if (resourceValid == false) {
			return null;
		}
		LOG.info(componentName + " of " + task + " use old assignment "
				+ oldAssignment);
