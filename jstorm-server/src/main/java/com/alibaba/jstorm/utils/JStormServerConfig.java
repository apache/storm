...
public class JStormServerConfig extends ConfigExtension {
	...
	/**
	 * How many cpu slot one supervisor can support , if it is null, it will be
	 * set by JStorm
	 */
	...
	public static Integer getSupervisorCpuSlotNum(Map conf) {
		return JStormUtils.parseInt(conf.get(SUPERVISOR_CPU_SLOT_NUM));
	}

	/**
	 * How many memory slot one supervisor can support , if it is null, it will
	 * be set by JStorm
	 */
	...
	public static Integer getSupervisorMemSlotNum(Map conf) {
		return JStormUtils.parseInt(conf.get(SUPERVISOR_MEM_SLOT_NUM));
	}

	/**
	 * # How much disk slot one supervisor can support # if it is null, it will
	 * use $(storm.local.dir)/worker_shared_data supervisor.disk.slot: null # if
	 * use multiple disks, it can be set as the following #supervisor.disk.slot:
	 * # - /disk0/jstorm/data # - /disk1/jstorm/data # - /disk2/jstorm/data # -
	 * /disk3/jstorm/data
	 */
	 ...
	 * 
	 * by default TOPOLOGY_TASK_ON_DIFFERENT_NODE_WEIGHT is 2
