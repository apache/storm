...
public class Zookeeper {
    ...
    public static LeaderLatchListener leaderLatchListenerImpl(final LeaderListenerCallback callback) throws UnknownHostException {
    ...
    public static ILeaderElector zkLeaderElector(Map conf, BlobStore blobStore, IStormClusterState clusterState, List<ACL> acls) throws UnknownHostException {
        return _instance.zkLeaderElectorImpl(conf, blobStore, clusterState, acls);
    }

    protected ILeaderElector zkLeaderElectorImpl(Map conf, BlobStore blobStore, IStormClusterState clusterState, List<ACL> acls) throws UnknownHostException {
        ...
        AtomicReference<LeaderLatchListener> leaderLatchListenerAtomicReference =
                new AtomicReference<>(leaderLatchListenerImpl(new LeaderListenerCallback(conf, zk, leaderLatchAtomicReference.get(), blobStore, clusterState, acls)));
        return new LeaderElectorImp(conf, servers, zk, leaderLockPath, id, leaderLatchAtomicReference,
            leaderLatchListenerAtomicReference, blobStore, clusterState, acls);
    }
