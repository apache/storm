...
def exec_storm_class(klass, jvmtype="-server", jvmopts=None, extrajars=None, args=None, fork=False, daemon=True,
                     daemon_name=""):
    jvmopts = jvmopts if jvmopts is not None else []
    extrajars = extrajars if extrajars is not None else []
    args = args if args is not None else []

    storm_log_dir = confvalue("storm.log.dir", [CLUSTER_CONF_DIR])
    if storm_log_dir is None or storm_log_dir == "nil":
...
def jar(jarfile, klass, *args):
    ...
    if transform_class is not None and transform_class != "nil":
        tmpjar = os.path.join(tempfile.gettempdir(), uuid.uuid1().hex + ".jar")
        exec_storm_class("org.apache.storm.daemon.ClientJarTransformerRunner", args=[transform_class, jarfile, tmpjar],
                         fork=True, daemon=False)
...
def get_log4j2_conf_dir():
    ...
    if storm_log4j2_conf_dir is None or storm_log4j2_conf_dir == "nil":
...
def print_commands():
    ...
    print("Configs can be overridden using one or more -c flags, e.g. "
...
COMMANDS = {"jar": jar, "kill": kill, "shell": shell, "nimbus": nimbus, "ui": ui, "logviewer": logviewer,
            ...
            "blobstore": blobstore, "get-errors": get_errors, "set_log_level": set_log_level,
            "kill_workers": kill_workers, "node-health-check": healthcheck, "sql": sql}
