...
public class RocksDbKey implements Comparable<RocksDbKey> {
    ...
    private static final Map<Byte, RocksDbKey> prefixMap = new HashMap<>();
    private byte[] key;
    ...
    static {
        ...
        for (KeyType type : EnumSet.allOf(KeyType.class)) {
            ...
            prefixMap.put(type.getValue(), key);
        }
    }
     ...
     */
    RocksDbKey(KeyType type, Integer metadataStringId) {
    ...
    static RocksDbKey getPrefix(KeyType type) {
        return prefixMap.get(type.getValue());
    }
     ...
     */
    static RocksDbKey createMetricKey(AggLevel aggLevel, Integer topologyId, long metricTimestamp, Integer metricId,
                                      Integer componentId, Integer executorId, Integer hostId, Integer port,
                                      Integer streamId) {
    ...
    Integer getTopologyId() {
        Integer val = ByteBuffer.wrap(key, 2, 4).getInt();
        return val;
    }
