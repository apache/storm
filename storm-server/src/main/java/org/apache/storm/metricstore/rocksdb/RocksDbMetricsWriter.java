 ...
 * method callback.
 */
public class RocksDbMetricsWriter implements Runnable {
    ...
    private void processInsert(Metric metric) throws MetricException {
        ...
        ListIterator li = store.aggBuckets.listIterator(store.aggBuckets.size());
        ...
        if (LOG.isDebugEnabled()) {
            ...
            LOG.debug("insert took " + duration / 1000 + " microSecs");
        }
    }
    ...
    private Integer storeMetadataString(KeyType type, String s, long metricTimestamp) throws MetricException {
        if (s == null) {
        ...
        if (stringMetadata != null) {
            ...
            stringMetadata.update(metricTimestamp, type);
            stringMetadataCache.put(s, stringMetadata);
            return stringMetadata.getStringId();
        }
        ...
        Integer stringId = getUniqueMetadataStringId();
    ...
    private Integer getUniqueMetadataStringId() throws MetricException {
        generateUniqueStringIds();
        Integer id = unusedIds.iterator().next();
        unusedIds.remove(id);
        return  id;
    }
    ...
    void shutdown() {
        ...
        // flush db to disk
        try {
            FlushOptions flushOps = new FlushOptions();
