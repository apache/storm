...
public class JCQueuePerfTest {
    ...
    private static void ackingProducerSimulation() {
        ...
        JCQueue spoutQ = new JCQueue("spoutQ", null, 1024, 0, 100, ws, "test", "test", Collections.singletonList(1000), 1000, registry);
        JCQueue ackQ = new JCQueue("ackQ", null, 1024, 0, 100, ws, "test", "test", Collections.singletonList(1000), 1000, registry);
    ...
    private static void producerFwdConsumer(int prodBatchSz) {
        WaitStrategyPark ws = new WaitStrategyPark(100);
        StormMetricRegistry registry = new StormMetricRegistry();
        JCQueue q1 = new JCQueue("q1", null, 1024, 0, prodBatchSz, ws, "test", "test",
                Collections.singletonList(1000), 1000, registry);
        JCQueue q2 = new JCQueue("q2", null, 1024, 0, prodBatchSz, ws, "test", "test", Collections.singletonList(1000), 1000, registry);
    ...
    private static void oneProducer2Consumers(int prodBatchSz) {
        WaitStrategyPark ws = new WaitStrategyPark(100);
        StormMetricRegistry registry = new StormMetricRegistry();
        JCQueue q1 = new JCQueue("q1", null, 1024, 0, prodBatchSz, ws, "test", "test", Collections.singletonList(1000), 1000, registry);
        JCQueue q2 = new JCQueue("q2", null, 1024, 0, prodBatchSz, ws, "test", "test", Collections.singletonList(1000), 1000, registry);
