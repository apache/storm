...
public class ConfigExtension {
    ...
    /**
     * Whether or not should the messaging transfer use disruptor, 
     * By default enable
     */
    ...
    public static Boolean isNettyEnableDisruptor(Map conf) {
        return JStormUtils.parseBoolean(conf.get(NETTY_ENABLE_DISRUPTOR_QUEUE), true);
    }
    ...
    /**
     * Usually, spout finish prepare before bolt, 
    ...
    public static long getMemSlotSize(Map conf) {
        return JStormUtils.parseLong(conf.get(MEM_SLOT_PER_SIZE), JStormUtils.SIZE_1_G);
    }
    ...
    /**
     * The disk slot assigned to the task
     * This configuration will be set by worker, application can't set it
     */
    ...
    /**
     * If component configuration set "use.userdefine.assignment", 
     * will try use user-defined assignment firstly
     * 
     * The task assigning algorith is as following
     * 
     * 1. if set USE_USERDEFINE_ASSIGNMENT, try use user-define assignment, 
     *    if the user-define assignment is available, use it
     * 2. if set USE_OLD_ASSIGNMENT, try use old assignment
     *    if the old assignment is available , use it
     * 3. assign the task from free-pool
     */
    protected static final String USE_USERDEFINE_ASSIGNMENT = "use.userdefine.assignment";

    public static void setUserDefineAssignment(Map conf, List<ResourceAssignment> userDefines) {
        conf.put(USE_USERDEFINE_ASSIGNMENT, userDefines);
    }

    /**
     * After submit topology to JStorm, getUserDefineAssignmentFromJson should be used
     * Before submit topology, getUserDefineAssignment should be used
     * 
     * @param conf
     * @return
     */
    ...
    /**
     * If component or topology configuration set "use.old.assignment", 
     * will try use old assignment firstly
     */
    ...
    /**
     * Force all tasks have been assigned in one node, 
     * which is to reduce network consumer
     * This is for samll topology
	 * This is conflict with TASK_ON_DIFFERENT_NODE
     */
     ...
     * The core task assign algorithm like this
     * supervisor.left.disk *  TOPOLOGY_DISK_WEIGHT + 
     * supervisor.left.cpu * TOPOLOGY_CPU_WEIGHT +
     * supervisor.left.mem * TOPOLOGY_MEM_WEIGHT +
     * supervisor.left.port * TOPOLOGY_PORT_WEIGHT
     * 
     * the task will be assigned to the highest supervisor
     * 
     * by default 
     * TOPOLOGY_DISK_WEIGHT is 3
     * TOPOLOGY_CPU_WEIGHT is 1
     * TOPOLOGY_MEM_WEIGHT is 1
     * TOPOLOGY_PORT_WEIGHT is 1
     */
    ...
    public static int getTopologyCpuWeight(Map conf) {
        return JStormUtils.parseInt(conf.get(TOPOLOGY_CPU_WEIGHT), DEFAULT_CPU_WEIGHT);
    }
    ...
    public static int getTopologyMemWeight(Map conf) {
        return JStormUtils.parseInt(conf.get(TOPOLOGY_MEM_WEIGHT), DEFAULT_MEM_WEIGHT);
    }
