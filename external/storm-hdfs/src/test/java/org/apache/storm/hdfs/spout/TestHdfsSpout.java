...
public class TestHdfsSpout {
	...
	@Test
	public void testIgnoreFilesFile() throws IOException, InterruptedException {
		...
		file1 = new Path(source + "/file1._COPYING_");
		createSeqFile(fs, file1, 5);
		file1 = new Path(source + "/file1.ignore");
		createSeqFile(fs, file1, 5);
		file1 = new Path(source + "/file2.seq");
		createSeqFile(fs, file1, 5);


		HdfsSpout spout = makeSpout(Configs.SEQ, SequenceFileReader.defaultFields);
		spout.setIgnoreSuffix(".ignore, ._COPYING_");
		Map conf = getCommonConfigs();
		openSpout(spout, 0, conf);
		...
		List<String> listDir = listDir(source);
		NavigableSet<String> set = new TreeSet<>(listDir);
		String firstMatch = set.ceiling(".ignore");
		Assert.assertNotNull(firstMatch);
		firstMatch = set.ceiling("._COPYING_");
		Assert.assertNotNull(firstMatch);
		
		Assert.assertEquals(2, listDir.size());
