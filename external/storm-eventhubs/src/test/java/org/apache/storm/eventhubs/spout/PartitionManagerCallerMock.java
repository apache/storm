...
public class PartitionManagerCallerMock {
	...
	public PartitionManagerCallerMock(String partitionId, long enqueueTimeFilter) {
		...
		conf.setReceiveEventsMaxCount(1);
	 ...
	 *            The commands are: r[N]: receive() called N times aX: ack(X) fY:
	 *            fail(Y)
	 * 
		...
		for (String cmd : cmds) {
			if (cmd.startsWith("r")) {
				...
				for (int i = 0; i < count; ++i) {
					EventHubMessage msg = pm.receive();
					if (msg == null) {
						ret.append("null,");
					} else {
						ret.append(msg.getMessageId().getOffset());
						ret.append(",");
					}
				}
